...
## returns the list of files, sorted by creation time
def makeListOfFiles(sourceDir, outputFile, logMetaData):
    ...
    newlist = sorted(listOfFiles, key=lambda d: d['date']) 
    return newlist
...
def combineFiles(fileList, outputFile):
    ...
    print("Combining files, this can take a while")
    outFile = os.path.splitext(outputFile)[0]
    outList = outFile+".txt"
    logText = outFile+".log"
    ...
    # attempts to preserve the metadata in the combined file.
    concat = ['ffmpeg', '-f', 'concat','-safe', '0', '-i', f'{outList}', 
              '-c', 'copy', '-map', '0:v', '-map', '0:a', 
              '-map', '0:3', '-copy_unknown', '-tag:2', 'gpmd', 
              f'{outputFile}']
...
def main():
    ...
    desc = """Utility to combine GoPro video files into one large file
              using the creation_time field in the metadata"""

    epil = """As well as the output.mp4, the files list, a log file and opionally 
              one json file is saved with the name of the output file but with 
              a different extention: .txt, .log and .json.
              
              Pre-exitsing files on the output will be deleted before being writtten anew.
    ...
    mp4Files = makeListOfFiles(sourceDir, outputFile, logMetaData)
    if not mp4Files:
        return        
